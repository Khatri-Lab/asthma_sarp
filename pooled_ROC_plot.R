...
IL_pooledROCPlot <- function(metaObject,filterObject,points=1000,weighting=TRUE,title=NULL,size=14,rounding=3,
                          smoothed=FALSE,auc1.thresh=0.99,bootReps=1000,minPoints=5,numCores=1,method="random", custom_colors){
  ...
  auc.summ = MetaIntegrator:::.aucROCframe(data.frame(roc.summ))
  ...
  # dataPal <- c(custom_colors[1:length(metaObject$originalData)])
  #names(dataPal) <- sapply(metaObject$originalData, function(x) x$formattedName)
  dataPal <- c(custom_colors, Pooled = "#000000")
  names(legendText) <- pooledStats$name
  legendText = c(legendText,Pooled = sprintf("Pooled AUC=%s (95%% CI %s-%s)",round(auc.summ,rounding),
                                    round(auc.lower,rounding),round(auc.upper,rounding)))
  plotData$name <- ordered(plotData$name, levels = names(dataPal))
